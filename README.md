...
이 파일은 다른 위치에 보관할 수 있으며, 파일을 열면 DKST macOS Notary Tool에서 불러옵니다. 이 파일은 특이한 작동 방식을 가지고 있습니다. B 지점에서 .DNT 파일을 열었을 때 A 지점에 앱 번들이 있는 경우, A 지점으로 .DNT 파일을 복사합니다. 이 기능은 클린 빌드 시 작업 내용이 삭제되는 불상사에 대비하여 백업본을 확보하기 위함입니다.
