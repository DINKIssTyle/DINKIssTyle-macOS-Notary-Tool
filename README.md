...
1. **Installer Background**: .pkg 인스톨러는 배경화면을 지원합니다. `Edit PKG-Installer-BG-TEMP.psd as an easy starting point for your background (opens read-only).` 안내문을 클릭하면 편집에 도움이 되는 Photoshop (.PSD)파일을 열수 있습니다. 수정한 .PSD 파일을 .PNG로 저장가능한 곳에 저장하고, `Choose...` 버튼을 클릭해 불러오세요.
...
1. **Layout Preset**: 2가지 프리셋과 수동 레이아웃을 선택할수 있습니다.
    ...
    1. Template 을 선택하면, `Edit DMG-BG-TEMP.psd as an easy starting point for your background (opens read-only).` 와 같이 각 레이아웃 배경 편집에 도움이 되는 안내문이 표시됩니다. 이를 클릭하면, Photoshop (.PSD)파일을 열수 있습니다. 수정한 .PSD 파일을 .PNG로 저장가능한 곳에 저장하고, Window Background `Choose...` 버튼을 클릭해 불러오세요.
...
## 자동 저장과 불러오기에 관하여
DKST macOS Notary Tool은 불러온 앱번들 폴더에 `.DNT`라는 저장 파일을 자동으로 생성합니다.
...
이 파일에는 앱 번들의 위치가 사용자 홈 폴더 기준 상대경로로 저장되므로 `.DNT` 파일은 어느 위치에 보관해도 됩니다. `.DNT` 파일을 열었을 때 저장된 위치에 앱 번들이 없으면 파일 선택 창이 열립니다. 새 앱 번들을 선택하면 해당 위치가 현재 `.DNT` 파일에 저장되며, 선택을 취소하면 앱 번들이 선택되지 않은 초기 화면으로 돌아갑니다. `.DNT` 파일을 앱 번들 옆으로 복사하지 않습니다.
